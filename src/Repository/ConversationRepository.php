...
class ConversationRepository extends ServiceEntityRepository
{
    ...
    */
    public function findConversationByParticipants(int $getId, int $getOtherId)
    {
    ...
    public function findConversationByUser(int $userId)
    {
        ...
        $qp->select('otherUser.Name', 'otherUser.Prename', 'otherUser.Picture' ,'c.id as conversationId', 'lm.content','lm.createdAt')
           ->innerJoin('c.participants', 'p', 'WITH', $qp->expr()->neq('p.user', ':user'))
           ->innerJoin('c.participants', 'me', 'WITH', $qp->expr()->eq('me.user', ':user'))
    ...
    public function checkIfUserisParticipant(int $conversation, int $getId)
    {
        $qb =$this->createQueryBuilder('c');
